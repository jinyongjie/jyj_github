...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
    ...
    protected FrameLayout mRefreshableViewHolder;
    ...
    protected void addRefreshableView(Context context, T refreshableView) {
        ...
        mRefreshableViewHolder.addView(refreshableView, ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        addView(mRefreshableViewHolder, new LinearLayout.LayoutParams(android.view.ViewGroup.LayoutParams.MATCH_PARENT, 0, 1.0f));
    }
    ...
    @Override
    protected void setDownRefreshingInternal(boolean doScroll, String title, String sub) {
        setRefreshingLabel(title);
        setLastUpdatedLabel(sub);
        if (mIndicatorIvBottom != null)
            mIndicatorIvBottom.setVisibility(View.GONE);
        super.setDownRefreshingInternal(doScroll, title, sub);
    }
    ...
    private void addIndicatorViews() {
        ...
        if (mode.canPullDown() && null == mIndicatorIvTop && !isRefreshing()) {
            // If the mode can pull down, and we don't have one set already
            mIndicatorIvTop = new IndicatorLayout(getContext(), Mode.PULL_DOWN_TO_REFRESH);
            FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT);
            params.leftMargin = getResources().getDimensionPixelSize(R.dimen.indicator_right_padding);
            params.gravity = Gravity.CENTER_VERTICAL | Gravity.LEFT;
            mHeaderLayout.addView(mIndicatorIvTop, params);

        } else if (!mode.canPullDown() && null != mIndicatorIvTop) {
            // If we can't pull down, but have a View then remove it
            mHeaderLayout.removeView(mIndicatorIvTop);
            mIndicatorIvTop = null;
        }

        if (mode.canPullUp() && null == mIndicatorIvBottom && !isRefreshing()) {
            // If the mode can pull down, and we don't have one set already
            ...
            params.leftMargin = getResources().getDimensionPixelSize(R.dimen.indicator_right_padding);
            params.gravity = Gravity.CENTER_VERTICAL | Gravity.LEFT;
            mFooterLayout.addView(mIndicatorIvBottom, params);

        } else if (!mode.canPullUp() && null != mIndicatorIvBottom) {
            // If we can't pull down, but have a View then remove it
            mFooterLayout.removeView(mIndicatorIvBottom);
    ...
    private void updateIndicatorViewsVisibility() {
        ...
        if (null != mIndicatorIvBottom) {
            if (!isRefreshing() && isReadyForPullUp()) {
                if (!mIndicatorIvBottom.isVisible() && !isQuickUpTo()) {
