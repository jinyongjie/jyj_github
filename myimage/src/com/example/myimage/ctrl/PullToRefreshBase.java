...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    if (mState == RELEASE_TO_REFRESH) {
                        ...
                        } else if (null != mOnBothRefreshListener) {
                            setRefreshingInternal(true);
                            if (mCurrentMode == Mode.PULL_DOWN_TO_REFRESH) {
                                mOnBothRefreshListener.onPullDownToRefresh();
                            } else if (mCurrentMode == Mode.PULL_UP_TO_REFRESH) {
                                mOnBothRefreshListener.onPullUpToRefresh();
                            }
                            return true;
                        }
    ...
    public boolean isQuickUpTo() {
        return mQuickUpTo;
    }

    public void setQuickUpTo(boolean mQuickUpTo) {
        this.mQuickUpTo = mQuickUpTo;
    }
     ...
     *            - Listener to be used when the Widget is set to Refresh
     */
    public final void setOnRefreshListener(OnBothRefreshListener listener) {
        mOnBothRefreshListener = listener;
    }
    ...
    public final void setDownRefreshing(boolean doScroll, String title, String sub) {
        if (!mPullToRefreshEnabled) {
            return;
        }
        if (!isRefreshing()) {
            if (null != mOnBothRefreshListener) {
                setDownRefreshingInternal(true, title, sub);
                mState = MANUAL_REFRESHING;
                mOnBothRefreshListener.onPullUpToRefresh();
            }
        }
    ...
    protected void addRefreshableView(Context context, T refreshableView) {
        addView(refreshableView, new LinearLayout.LayoutParams(android.view.ViewGroup.LayoutParams.MATCH_PARENT, 0, 1.0f));
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrHeaderBackground)) {
            ...
            if (null != background) {
                ...
                mFooterLayout.setBackgroundDrawable(background);
            }
        }
     ...
     * @author Chris Banes
     */
    public static interface OnBothRefreshListener {
