...
public class Folder_GridMode extends GridView implements Folder_Mode {
	...
	public Folder_GridMode(Context context, android.util.AttributeSet attrs) {
		super(context, attrs);
	...
	public void init(boolean selmode)
	{
		...
		else
		{
			setClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					// TODO Auto-generated method stub
					for(int i = 0;i<Folder_GridMode.this.getChildCount();i++)
					{
						DirItemGrid item = (DirItemGrid) Folder_GridMode.this.getChildAt(i);
						item.setSel(false);
					}
